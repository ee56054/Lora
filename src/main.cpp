...
// #define RESET_PIN 18  // GPIO pin for reset (using wiringOP)
// #define DIO4_PIN 6    // GPIO pin for transmit enable - DIO4 (using wiringOP)
// #define DIO1_PIN 16   // GPIO pin for interrupt - DIO1 (using wiringOP)
// #define BUSY_PIN 20   // GPIO pin for busy status (using wiringOP)
// #define CS_PIN 21     // GPIO pin for chip select (using wiringOP)
...
#define CS_PIN 27   // GPIO pin for chip select (using wiringOP)s

// Global SPI file descriptor
static int spi_fd = -1;

// GPIO helper functions using wiringOP
...
int main(int argc, char *argv[]) {
  ...
  if (argc > 1 && std::string(argv[1]) == "rx") {
    receiver_mode = true;
  }

  std::cout << "Hello, World from CMake project with SX126X driver!"
            << std::endl;
  std::cout << "Mode: " << (receiver_mode ? "RECEIVER" : "TRANSMITTER")
            << std::endl;
  ...
  // Initialize wiringOP
  if (wiringPiSetup() == -1) {
    std::cerr << "Failed to initialize wiringOP" << std::endl;
    return 1;
  }
