...
bool LoraConfig::load_from_file(const std::string &filepath) {
  ...
  if (!file.is_open()) {
    std::cerr << "Failed to open config file for reading: " << filepath
              << std::endl;
    return false;
  }
  ...
  try {
    ...
    if (j.contains("frequency"))
      frequency = j["frequency"].get<uint32_t>();
    if (j.contains("tx_power"))
      tx_power = j["tx_power"].get<int8_t>();
    if (j.contains("spreading_factor"))
      spreading_factor = str_to_sf(j["spreading_factor"].get<std::string>());
    if (j.contains("bandwidth"))
      bandwidth = str_to_bw(j["bandwidth"].get<std::string>());
    if (j.contains("coding_rate"))
      coding_rate = str_to_cr(j["coding_rate"].get<std::string>());
    if (j.contains("preamble_length"))
      preamble_length = j["preamble_length"].get<uint16_t>();
    if (j.contains("rx_timeout"))
      rx_timeout = j["rx_timeout"].get<uint32_t>();

    if (j.contains("modbus_enabled"))
      modbus_enabled = j["modbus_enabled"].get<bool>();
    if (j.contains("modbus_slave_id"))
      modbus_slave_id = j["modbus_slave_id"].get<int>();
    if (j.contains("modbus_address_devices")) {
